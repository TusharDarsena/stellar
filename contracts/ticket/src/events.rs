...
pub fn emit_ticket_purchased(env: &Env, ticket_id: &Symbol, buyer: &Address, event_id: &Symbol) {
    env.events().publish(
...
pub fn emit_funds_released(env: &Env, event_id: &Symbol, organizer: &Address, amount: i128) {
    env.events().publish(
...
pub fn emit_refund(env: &Env, ticket_id: &Symbol, attendee: &Address, amount: i128) {
    env.events().publish(
...
pub fn emit_ticket_used(env: &Env, ticket_id: &Symbol) {
    env.events()
        .publish((symbol_short!("tk_used"), ticket_id.clone()), ());
}

pub fn emit_restricted_transfer(env: &Env, ticket_id: &Symbol, new_owner: &Address) {
    env.events().publish(
